...
import {
  ...
  refreshTelecomData,
  type TelecomDataCache,
  type TelecomUsage,
} from "./telecom"

const LIGHT_DARK_BACKGROUND = { light: "#FFFFFF", dark: "#000000" }
const SMALL_FEE_COLOR = "#0C54D9"
const MEDIUM_FEE_COLOR = "#0A4B9D"
...
function cacheIsDue(
  ...
  return Date.now() - updatedAt >= intervalMinutes * 60_000
}

async function prepareWidgetSnapshot(): Promise<WidgetSnapshot> {
  // Read the last successful value first. Network errors never erase this cache.
  ...
  const refreshed = await refreshTelecomData()
  if (refreshed.ok) {
    return {
      data: refreshed.data,
  ...
  return {
    data: refreshed.cachedData ?? cache?.data ?? EMPTY_TELECOM_USAGE,
    updatedAt:
...
function SmallRow(props: SmallRowProps) {
  return (
    ...
    >
      ...
      {props.logo ? (
        <Image
          filePath={`${Script.directory}/telecom-logo.png`}
          resizable
          scaleToFit
...
function Ring(props: { percent: number; color: string; icon: string }) {
  return (
...
function MediumCell(props: MediumCellProps) {
  return (
    <VStack
      alignment="center"
      spacing={0}
...
function SmallWidget(props: WidgetSnapshot) {
  ...
  return (
    <VStack
      ...
      frame={{ maxWidth: "infinity", maxHeight: "infinity" }}
      padding={12}
      widgetBackground={LIGHT_DARK_BACKGROUND}
    >
      <SmallRow
        title="剩余话费"
        value={formatValue(data.balance)}
        unit="元"
        color={SMALL_FEE_COLOR}
        logo
        stale={props.stale}
      />
      <Spacer />
      <SmallRow
        title="剩余流量"
        value={formatValue(data.flowRemaining)}
        unit="GB"
        color={FLOW_COLOR}
      ...
      <SmallRow
        title="剩余语音"
        value={formatValue(data.voiceRemaining, 0)}
        unit="分钟"
        color={VOICE_COLOR}
...
async function run() {
  ...
  const cachedTime = new Date(snapshot.updatedAt).getTime()
  const nextRefresh =
    !snapshot.stale && Number.isFinite(cachedTime)
      ? Math.max(Date.now() + 60_000, cachedTime + intervalMs)
      : Date.now() + intervalMs
  const view =
